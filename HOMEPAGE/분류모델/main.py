...
def doit(newmap, frcount, ROI_range):
    ...
    for i in range(1, frcount+1):
        ...
        canny_img = canny(blur_img, 120, 150)  # Canny edge 알고리즘

        global vertices
        vertices = np.array([ROI_range], dtype=np.int32)
        ROI_img = region_of_interest(canny_img, vertices)

        # line_arr = cv2.HoughLinesP(ROI_img, 1, np.pi / 180, 50, None, 50, 10)
        line_arr = cv2.HoughLinesP(ROI_img, 1, np.pi / 180, 20, 10, 20, 10)
        # hough_lines(ROI_img, 1, 1 * np.pi/180, 20, 10, 20) # 허프 변환
        #[x1, y1, x2, y2]
        ...
        # 필터링된 직선 버리기
        L_lines, R_lines = line_arr[(
            slope_degree > 0), :], line_arr[(slope_degree < 0), :]
...
def rl_detec(chaseon):
    ...
    for j in chaseon:
        for i in j:
            ...
            if math.tan(math.pi * (50/180)) <= a <= math.tan(math.pi * (85/180)):  # 차선의 기울기가 85도를 넘어가면
                count_L += 1
                if count_L >= 3:  # 5프레임 이상 지속되면
                    change_L += 1

                # elif 3 < count_L < 5:
                #     return 'yes_sL'

            if math.tan(math.pi * (95/180)) <= a <= math.tan(math.pi * (140/180)):
                # math.tan(math.pi * (180/180)) <= a <= math.tan(math.pi * (130/180)):
                count_R += 1
                if count_R >= 5:
                    change_R += 1
    ...
    # if (change > change_L) or (change > change_R):
    #    return 'yes'
...
def SAM_572(search_range, newmap, aV, a):
    av = aV
    temp1 = [0, 0, 0]  # 왼쪽
    temp2 = [0, 0, 0]  # 오른쪽
    temp = []
    ...
    for h in range(3):
        temp1[h] = temp1[h]//((search_range[1][0]-search_range[0][0]+search_range[3]
                              [0]-search_range[2][0])*(search_range[2][1]-search_range[0][1])//4)  # 평균 내기
        temp2[h] = temp2[h]//((search_range[1][0]-search_range[0][0]+search_range[3]
                              [0]-search_range[2][0])*(search_range[2][1]-search_range[0][1])//4)
    temp.append(temp1)
    temp.append(temp2)
    av.append(temp)
    return av
