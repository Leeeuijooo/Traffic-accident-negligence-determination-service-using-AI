...
def gradient(a): #기울기
    g=(np.array(a[3])-np.array(a[1]))/(np.array(a[2])-np.array(a[0]))   #delta y/ delta x
    return g
...
av=['sam']
tempchaseon, vertices=doit(newmap,5,ROI_range)  #5프레임 정도 미리 차선 따놓기
...
cs=analcoc(chaseon)      #내가 차선 변경 하였는지 #yes or no
print('cs=',cs)
